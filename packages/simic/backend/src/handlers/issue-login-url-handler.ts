...
export const issueLoginUrlHandler: (deps: {
  ...
  try {
    ...
    if (
      (
        await executeQuery(
          SQL`SELECT * FROM USERS WHERE discord_id = ${req.body.discordId};`
        )
      ).length === 0
    ) {
      await executeQuery(
        SQL`INSERT INTO users (discord_id, current_login_id) VALUES (${req.body.discordId}, ${loginId});`
      );
    } else {
      await executeQuery(
        SQL`UPDATE users SET current_login_id = ${loginId} WHERE discord_id = ${req.body.discordId};`
      );
      await executeQuery(
        SQL`DELETE FROM refresh_tokens WHERE discord_id = ${req.body.discordId};`
      );
    }
