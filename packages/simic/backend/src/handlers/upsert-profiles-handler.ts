...
export const upsertProfileHandler = (deps: {
  ...
  try {
    ...
    try {
      ...
      await executeQuery(
        SQL`DELETE FROM profiles WHERE profiles.discord_id = ${discordId};`
      );
      for (const profile of req.body.profiles) {
        await executeQuery(
          SQL`INSERT INTO profiles (discord_id, stream_key) VALUES (${discordId}, ${profile.streamKey});`
        );
      }
