...
type Token = string;
export const refreshTokenHandler = (deps: {
  ...
  try {
    ...
    const [token] = await executeQuery(
      SQL`SELECT * FROM refresh_tokens WHERE token = ${req.body.refreshToken} AND activated = true AND datetime('now', '-6 months') > created_at;`
    );
    if (!token) {
    ...
    try {
      ...
      await executeQuery(
        SQL`UPDATE refresh_tokens SET created_at = datetime('now'), token = ${newToken} WHERE token = ${req.body.refreshToken} AND activated = true AND datetime('now', '-6 months') > created_at;`
      );
