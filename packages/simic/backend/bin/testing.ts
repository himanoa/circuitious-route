...
const authorizeEndPoint = (loginId: string) =>
  `${cli.input[0]}/${loginId}/authorize`;
...
async function successStory() {
  ...
  const loginResponse = await fetch(loginUrlEndPoint, {
    method: "post",
    body: JSON.stringify({ discordId }),
    headers: { "Content-Type": "application/json" },
  });
  const { loginId } = await loginResponse.json();
  const authorizeResponse = await fetch(authorizeEndPoint(loginId), {
    method: "post",
  });
  ...
  const newVerifyResponse = await fetch(verifyEndPoint, {
    method: "get",
    headers: { Authorization: `Barer ${accessToken}` },
  });
