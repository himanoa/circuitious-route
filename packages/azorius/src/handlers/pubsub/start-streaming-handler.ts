...
export const startStreamingHandler = (deps: Dependencies) => (event: StartStreamingEvent) => {
  const { discordId } = event.payload
  const voiceChannels = deps.discordClient.channels.cache.filter(c => c.type === "voice")
  ...
  if(streamerUserJoinedVoiceChannel && isVoiceChannel(streamerUserJoinedVoiceChannel)) {
    return streamerUserJoinedVoiceChannel.join()
  }
  deps.logger.info("ボイスチャンネルに配信開始したユーザーが存在しませんでした")
}
