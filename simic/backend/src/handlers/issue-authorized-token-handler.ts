...
export const issueAuthorizedTokenHandler:(deps: {
  ...
  try {
    ...
    const [user] = await executeQuery(SQL`SELECT * FROM users WHERE current_login_id = ${req.params.loginId};`)
    if(!user) {
    ...
    if(refreshToken.length === 0) {
      ...
      await executeQuery(SQL`INSERT INTO refresh_tokens (discord_id, token, activated, created_at) VALUES (${user.discord_id}, ${refreshToken}, 1, strftime('%s', 'now'));`)
      res.status(201).json({
